...
function s.initial_effect(c)
	...
	local e4=c:DriveEffect(-4,1159,nil,nil,nil,nil,aux.NOT(s.qcon),nil,s.settg,s.setop):Clone()
...
function s.negcon(e,tp,_,_,ev,re,_,rp)
	if rp~=1-tp or not (re:IsActiveType(TYPE_TRAP) and Duel.IsChainNegatable(ev)) then return false end
...
function s.filter(c)
	if not (c:IsSSetable() and c:IsType(TYPE_TRAP)) then return false end
	for i=0,2 do if aux.IsCodeListed(c,212111806+i) then return true end end
	return false
end
