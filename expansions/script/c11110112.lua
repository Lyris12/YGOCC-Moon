...
function cm.target(e,c)
	return c:IsSetCard(0x2a7) and c:IsType(TYPE_SYNCHRO+TYPE_XYZ+TYPE_LINK)
end
function cm.togfilter_rel(c)
	return c:IsSetCard(0x2a7) and c:IsReleasable()
end
...
function cm.togfilter_tog(c)
	return c:IsSetCard(0x2a7) and c:IsAbleToGrave()
end
...
function cm.rltg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(cm.rlfilter,tp,LOCATION_MZONE,0,1,nil) and eg:IsExists(Card.IsSetCard,1,nil,0x2a7) and Duel.IsPlayerCanDraw(tp,1) end
