...
function c249000121.initial_effect(c)
	...
	e2:SetCondition(c14778250.spcon)
	e2:SetOperation(c14778250.spop)
	c:RegisterEffect(e2)
end
...
function c249000121.disfilter(c)
	return c:GetOriginalLevel()>=7 and c:GetOriginalLevel()<=10 and not c:IsSummonableCard() and c:IsType(TYPE_MONSTER)
end
function c249000121.spcon(e,c)
	if c==nil then return true end
	local tp=c:GetControler()
	return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(c249000121.disfilter,tp,LOCATION_HAND,0,1,c)
end
function c249000121.spop(e,tp,eg,ep,ev,re,r,rp,c)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DISCARD)
	local g=Duel.SelectMatchingCard(tp,c249000121.disfilter,tp,LOCATION_HAND,0,1,1,c)
