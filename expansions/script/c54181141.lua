...
function s.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then
		...
		return Duel.IsExistingMatchingCard(s.cfilter,tp,LOCATION_GRAVE+LOCATION_MZONE,0,2,nil)
	end
	...
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	Duel.Remove(Duel.SelectMatchingCard(tp,s.cfilter,tp,LOCATION_GRAVE+LOCATION_MZONE,0,2-#g,2-#g,nil),POS_FACEUP,REASON_COST)
end
...
function s.tg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,Duel.SelectTarget(tp,s.filter,tp,LOCATION_REMOVED+LOCATION_GRAVE,0,1,1,nil),1,0,0)
end
