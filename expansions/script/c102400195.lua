...
function cid.initial_effect(c)
	c:EnableReviveLimit()
	aux.AddFusionProcFunFunRep(c,cid.mfilter1,cid.mfilter2,1,64,false,true)
...
function cid.mfilter2(c,fc,sub,mg,sg)
	return c:IsLevelBelow(4) and (not sg or sg:FilterCount(aux.TRUE,c)==0 or sg:Filter(Card.IsLevelAbove,c,1):CheckWithSumGreater(Card.GetLevel,fc:GetLevel()-c:GetLevel()))
end
function cid.target(e,tp,eg,ep,ev,re,r,rp,chk)
	local sg=Duel.GetFieldGroup(tp,0,LOCATION_ONFIELD)
	if chk==0 then return e:GetHandler():GetMaterialCount()>0 and #sg>0 end
