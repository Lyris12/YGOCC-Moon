--Totems delle Lucertole Dimenticate
...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	--ATK
	c:UpdateATKField(200,nil,LOCATION_MZONE,0,aux.TargetBoolFunction(Card.IsRace,RACE_DINOSAUR+RACE_REPTILE))
	--splimit
...
function s.thfilter(c)
	return c:IsMonster() and c:IsRace(RACE_DINOSAUR+RACE_REPTILE) and c:IsAbleToHand()
end
...
function s.splimit(e,c)
	return not c:IsRace(RACE_DINOSAUR+RACE_REPTILE)
end
...
function s.tdfilter(c)
	return c:IsMonster() and c:IsRace(RACE_DINOSAUR+RACE_REPTILE) and c:IsAbleToDeck()
end
