...
function s.initial_effect(c)
    ...
    e3:SetTarget(cid.ptg)
    e3:SetOperation(cid.pop)
    c:RegisterEffect(e3)
end
...
function cid.ptg(e,tp,eg,ep,ev,re,r,rp,chk)
    if chk==0 then return Duel.IsExistingMatchingCard(cid.pfilter,tp,LOCATION_EXTRA,0,2,nil) end
...
function cid.pop(e,tp,eg,ep,ev,re,r,rp)
    ...
    local g=Duel.SelectMatchingCard(tp,cid.pfilter,tp,LOCATION_EXTRA,0,2,2,nil)
