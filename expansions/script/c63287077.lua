...
function s.matfilter(c)
	return c:IsFusionCode(63287015) and c:GetEquipGroup() and c:GetEquipGroup():IsExists(s.eqmatfilter,1,nil)
end
...
function s.filter(c)
	return c:IsFaceup() and (not c:IsAttack(0) or not c:HasFlagEffect(id))
end
...
function s.disop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and tc:IsRelateToEffect(e) then
		if tc:IsFaceup() then
			...
			e2:SetCode(EFFECT_GLITCHY_CANNOT_CHANGE_ATK)
			e2:SetValue(s.value)
...
function s.value(e,re,r)
	return r&REASON_EFFECT>0 and re:GetOwner()~=e:GetOwner()
end