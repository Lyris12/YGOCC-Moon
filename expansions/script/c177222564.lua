...
function s.initial_effect(c)
	...
	c:RegisterEffect(e2)
	--During your opponent's turn (Quick Effect): You can target 1 Set card your opponent controls; attach it to this card as material.
	...
	c:RegisterEffect(e3)
	--You can detach 1 material from this card; Set 1 "Oniritron" Trap directly from your Deck, except an "Oniritron Device" card.
...
function s.filter(c)
	return c:IsAbleToChangeControler()
		and not c:IsType(TYPE_TOKEN) and c:IsFacedown()
end
...
function s.setfilter(c)
	return c:IsSSetable() and c:IsSetCard(0x721) and not c:IsSetCard(0x1721) and c:IsType(TYPE_TRAP)
end
