...
function c249000057.initial_effect(c)
	...
	e2:SetCondition(c249000057.spcon)
	c:RegisterEffect(e2)
end
function c249000057.costfilter(c)
	return c:IsSetCard(0xA5) and c:IsAbleToRemoveAsCost()
end
function c249000057.costfilter2(c,e)
	return c:IsSetCard(0xA5) and not c:IsPublic()
end
function c249000057.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return (Duel.IsExistingMatchingCard(c249000057.costfilter,tp,LOCATION_GRAVE,0,1,nil)
	or Duel.IsExistingMatchingCard(c249000057.costfilter2,tp,LOCATION_HAND,0,1,nil)) end
	local option
	if Duel.IsExistingMatchingCard(c249000057.costfilter2,tp,LOCATION_HAND,0,1,nil)  then option=0 end
	...
	and Duel.IsExistingMatchingCard(c249000057.costfilter2,tp,LOCATION_HAND,0,1,nil) then
		option=Duel.SelectOption(tp,526,1102)
	end
	if option==0 then
		g=Duel.SelectMatchingCard(tp,c249000057.costfilter2,tp,LOCATION_HAND,0,1,1,nil,e)
...
function c249000057.tgfilter(c)
...
function c249000057.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c249000057.tgfilter,tp,LOCATION_DECK+LOCATION_EXTRA,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TOGRAVE,nil,1,tp,LOCATION_DECK+LOCATION_EXTRA)
end
function c249000057.operation(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local g=Duel.SelectMatchingCard(tp,c249000057.tgfilter,tp,LOCATION_DECK+LOCATION_EXTRA,0,1,1,nil)
	if g:GetCount()>0 then
		...
		if c:IsRelateToEffect(e) and c:IsFaceup() and Duel.SendtoGrave(g,REASON_EFFECT)~=0 then
			...
			e3:SetOperation(c249000057.rstop)
			c:RegisterEffect(e3)
		end
	end
end
function c249000057.rstop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
