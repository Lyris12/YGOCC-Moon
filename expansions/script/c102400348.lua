...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	aux.AddContactFusionProcedure(c,aux.FilterBoolFunction(s.mfilter,c:GetControler()),LOCATION_HAND+LOCATION_ONFIELD,LOCATION_ONFIELD,Duel.Release,REASON_COST)
...
function s.mfilter(c,tp)
	return Duel.GetReleaseGroup(tp,true):IsContains(c)
end
function s.read(c)
