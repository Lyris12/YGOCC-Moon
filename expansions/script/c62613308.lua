...
function s.initial_effect(c)
	--xyz summon
	aux.AddXyzProcedure(c,aux.FilterBoolFunction(Card.IsSetCard,ARCHE_NIGHTSHADE),4,2,s.ovfilter,aux.Stringid(id,0),2,s.xyzop)
	c:EnableReviveLimit()
	--negate
...
function s.ovfilter(c,e,tp,xyzc)
	return c:IsFaceup() and c:IsSetCard(ARCHE_NIGHTSHADE) and c:IsXyzType(TYPE_SYNCHRO) and c:IsXyzLevel(xyzc,4)
end
