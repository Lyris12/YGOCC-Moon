...
function s.filter(c)
	return c:IsSetCard(0x2c2) and (c:IsFaceup() or not c:IsLocation(LOCATION_EXTRA)) and c:IsPandemoniumSSetable()
end
function s.settg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_SZONE)>0 and Duel.IsExistingMatchingCard(s.filter,tp,LOCATION_DECK+LOCATION_EXTRA,0,1,nil) end
end
