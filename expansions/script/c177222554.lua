...
function s.initial_effect(c)
	...
	c:RegisterEffect(e2)
	--If this card is Special Summoned: You can target up to 2 cards you control; destroy them, and if you do, add "Temporius" cards with different names from your Deck to your hand equal to the number of cards destroyed.
...
function s.sumcon(e,c)
	return not Duel.CheckPendulumZones(e:GetHandlerPlayer())
end
function s.tlfilter(c,e,mg)
	local tp=c:GetControler()
	local ef=e:GetHandler():GetFuture()
	return c:IsLevelBelow(ef-1) and (c:IsAttribute(ATTRIBUTE_EARTH) or c:IsType(TYPE_PENDULUM))
end
function s.cfilter(c,tp)
	return c:IsFaceup() and c:IsType(TYPE_TIMELEAP) and c:IsControler(tp) and c:IsSummonType(SUMMON_TYPE_TIMELEAP)
end
function s.atkcon(e,tp,eg,ep,ev,re,r,rp)
	return eg:IsExists(s.cfilter,1,nil,tp)
end
function s.filter(c)
	return c:IsFaceup() and c:IsType(TYPE_TIMELEAP)
end
...
function s.thfilter(c)
	return c:IsSetCard(0x723) and not c:IsCode(id) and c:IsAbleToHand()
end
