...
function c249000392.racefilter(c,race)
...
function c249000392.filter(c,e,tp,m)
	if not c:IsType(TYPE_XYZ) or c:GetRank()<5 or not c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_XYZ,tp,false,false)
		or not Duel.IsExistingMatchingCard(c249000392.racefilter,tp,LOCATION_HAND+LOCATION_GRAVE+LOCATION_REMOVED,0,1,nil,c:GetRace()) then return false end
...
function c249000392.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chk==0 then
		local mg=Duel.GetMatchingGroup(c249000392.matfilter,tp,LOCATION_HAND+LOCATION_MZONE,0,e:GetHandler())
...
function c249000392.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc then
		local rg=Duel.GetMatchingGroup(c249000392.racefilter,tp,LOCATION_HAND+LOCATION_GRAVE+LOCATION_REMOVED,0,nil,tc:GetRace())
