...
function cid.rmfilter(c,e,tp)
    return c:IsFaceup() and  c:IsSetCard(0xc50)  and c:IsAbleToRemoveAsCost()
        and Duel.IsExistingTarget(cid.filterx,tp,LOCATION_EXTRA+LOCATION_GRAVE,0,1,c,e,tp)
end
...
if chk==0 then return Duel.IsExistingMatchingCard(cid.rmfilter,tp,LOCATION_EXTRA,0,1,nil,e,tp) end
local g=Duel.SelectMatchingCard(tp,cid.rmfilter,tp,LOCATION_EXTRA,0,1,1,nil,e,tp)
    Duel.Remove(g,POS_FACEUP,REASON_COST)
end
function cid.thtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
  if chk==0 then return Duel.IsExistingMatchingCard(cid.filterx,tp,LOCATION_EXTRA+LOCATION_GRAVE,0,1,nil) end
    Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_EXTRA+LOCATION_GRAVE)
end
function cid.thop(e,tp,eg,ep,ev,re,r,rp)
    Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
    local g=Duel.SelectMatchingCard(tp,cid.filterx,tp,LOCATION_EXTRA+LOCATION_GRAVE,0,1,1,nil)
...
function cid.filter(c,e,tp,b1,setcode)
...
function cid.filter2(c,e,tp,b2,setcode)
...
function cid.target(e,tp,eg,ep,ev,re,r,rp,chk)
    ...
    if chk==0 then return (b1 or b2)
        and Duel.IsExistingMatchingCard(cid.filter,tp,LOCATION_DECK,0,1,nil,e,tp,b1,nil)
        and Duel.IsExistingMatchingCard(cid.filter2,tp,LOCATION_DECK,0,1,nil,e,tp,b1,nil) end
...
function cid.operation(e,tp,eg,ep,ev,re,r,rp)
    ...
    local g1=Duel.GetMatchingGroup(cid.filter,tp,LOCATION_DECK,0,nil,e,tp,b1,nil)
    local g2=Duel.GetMatchingGroup(cid.filter2,tp,LOCATION_DECK,0,nil,e,tp,b1,nil)
