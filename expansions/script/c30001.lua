...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 or not c:IsRelateToChain() then end
...
function s.chcon2(e,tp,eg,ep,ev,re,r,rp)
	...
	return rc:IsRelateToBattle() and rc:IsStatus(STATUS_OPPO_BATTLE)
		and rc:IsFaceup() and rc:IsMonster(TYPE_PENDULUM) and rc:IsSetCard(0x10ec) and rc:IsControler(tp)
end
...
function s.mfield(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	Duel.SetCustomOperationInfo(0,CATEGORY_DEFCHANGE,g,#g,tp,LOCATION_MZONE,500)
end
