--Infernalizzatrice del Cielo Brakadiano Citrinoartiglieria
...
function s.cf(c,tp)
	return c:IsFaceup() and c:IsMonster() and c:IsAttribute(ATTRIBUTE_FIRE) and c:IsRace(RACE_DRAGON) and Duel.GetMZoneCount(tp,c)>0 and c:IsAbleToRemoveAsCost()
end
...
function s.destg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsExistingTarget(s.f1,tp,LOCATION_ONFIELD,0,1,e:GetHandler()) and Duel.IsExistingTarget(aux.NegateAnyFilter,tp,0,LOCATION_ONFIELD,1,nil) end
...
function s.desop(e,tp,eg,ep,ev,re,r,rp)
	...
	local tc1,tc2=tg:GetFirst(),tg:GetNext()
	if e:GetLabelObject()~=tc1 then tc1,tc2=tc2,tc1 end
	if not tc1 or e:GetLabelObject()~=tc1 or not s.f1(tc1) then return end
	if Duel.Destroy(tc1,REASON_EFFECT)>0 then
		if not tc2 or not aux.NegateAnyFilter(tc2) then return end
		Duel.Negate(tc2,e)
	end
end
