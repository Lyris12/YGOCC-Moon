...
function s.initial_effect(c)
	...
	c:RegisterEffect(e2)
	aux.AddPreTransformationCheck(c,e2,s.recon(ARCHE_DREAMY_FOREST,GYChk))
end
...
function s.tffilter(c,tp,arche,se)
	return c:IsFaceup() and c:IsControler(tp) and c:IsLocation(LOCATION_MZONE) and c:IsSetCard(arche) and (se==nil or c:GetReasonEffect()~=se)
end
function s.recon(arche,se)
	return	function(e,tp,eg,ep,ev,re,r,rp)
				return eg:IsExists(s.tffilter,1,nil,tp,arche,se)
			end
end
