...
function cid.initial_effect(c)
    ...
    e2:SetTarget(cid.fntg)
    e2:SetOperation(cid.fnop)
    c:RegisterEffect(e2)
end
...
function cid.fntg(e, tp, eg, ep, ev, re, r, rp, chk)
	if chk==0 then
		local chkf=tp
		...
		return res and Duel.IsExistingMatchingCard(cid.tdfilter, tp, LOCATION_HAND+LOCATION_GRAVE, 0, 1, nil)
    end
    Duel.SetOperationInfo(0, CATEGORY_TODECK, nil, 1, tp, LOCATION_HAND+LOCATION_GRAVE)
	Duel.SetOperationInfo(0, CATEGORY_SPECIAL_SUMMON, nil, 1, tp, LOCATION_EXTRA)
end
function cid.fnop(e, tp, eg, ep, ev, re, r, rp)
    Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_TODECK)
    local g=Duel.SelectMatchingCard(tp, cid.tdfilter, tp, LOCATION_HAND+LOCATION_GRAVE, 0, 1, 1, nil)
    if g:GetCount()==0 then return end
    if Duel.SendtoDeck(g, tp, 2, REASON_EFFECT)~=0 and Duel.ShuffleDeck(tp)~=0 then
        local chkf=tp
