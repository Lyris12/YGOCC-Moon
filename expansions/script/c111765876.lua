function c.initial_effect(c)
	--link summon
	aux.AddLinkProcedure(c,nil,2,2,c111765881.lcheck)
	c:EnableReviveLimit()
	--scrapdragon
	...
	e2:SetCountLimit(1,)
	e2:SetCondition(c111765881.thcon)
	e2:SetTarget(c111765881.thtg)
	e2:SetOperation(c111765881.thop)
	c:RegisterEffect(e2)
end
	--link summon
function c111765881.lcheck(g,lc)
...
--float
function c111765881.thcon(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	return c:IsPreviousLocation(LOCATION_MZONE) and c:IsSummonType(SUMMON_TYPE_LINK)
end
function c111765881.spfilter(c,e,tp)
	return c:IsSetCard(0x736) and c:IsCanBeSpecialSummoned(e,0,tp,false,false) and not c:IsType(TYPE_LINK)
end
function c111765881.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(c111765881.spfilter,tp,LOCATION_GRAVE,0,1,nil,e,tp) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_GRAVE)
end
function c.thop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(c111765881.spfilter),tp,LOCATION_GRAVE,0,1,1,nil,e,tp)
