...
function cid.posfilter1(c)
	return (c:IsType(TYPE_SPELL+TYPE_TRAP) or c:IsType(TYPE_LINK))
end
...
function cid.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsExistingTarget(cid.posfilter,tp,LOCATION_MZONE,0,1,nil)
		and Duel.IsExistingTarget(cid.posfilter1,tp,0,LOCATION_ONFIELD,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_POSCHANGE)
	local g1=Duel.SelectTarget(tp,cid.posfilter,tp,LOCATION_MZONE,0,1,1,nil)
	e:SetLabelObject(g1:GetFirst())
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DISABLE)
	local g2=Duel.SelectTarget(tp,cid.posfilter1,tp,0,LOCATION_ONFIELD,1,1,nil)
...
function cid.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local tg=g:GetFirst()
	if g:GetCount()>0 and Duel.ChangePosition(tg,POS_FACEUP_ATTACK+POS_FACEUP_DEFENSE)~=0 then
		if tg:IsType(TYPE_FLIP) and Duel.SendtoGrave(g:GetNext(),REASON_EFFECT)~=0
