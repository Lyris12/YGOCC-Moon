...
function c249001146.initial_effect(c)
	...
	e2:SetCountLimit(1)
	e2:SetTarget(c249001146.target)
	e2:SetOperation(c249001146.operation)
	c:RegisterEffect(e2)
end
...
function c249001146.filter2(c)
	return c:IsFaceup() and c:IsLevelAbove(1)
end
function c249001146.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c249001146.filter2(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c249001146.filter2,tp,LOCATION_MZONE,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FACEUP)
	local g=Duel.SelectTarget(tp,c249001146.filter2,tp,LOCATION_MZONE,0,1,1,nil)
	Duel.Hint(HINT_SELECTMSG,tp,HINGMSG_LVRANK)
	local lv=Duel.AnnounceLevel(tp,1,5,g:GetFirst():GetLevel())
	e:SetLabel(lv)
end
function c249001146.operation(e,tp,eg,ep,ev,re,r,rp)
	local tc=Duel.GetFirstTarget()
