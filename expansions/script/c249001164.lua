...
function c249001164.spfilter1(c,e,tp,lvt)
	...
	return c:IsLevelBelow(lvt) and c:IsSetCard(0x22D) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
		and Duel.IsExistingMatchingCard(c249001164.spfilter2,tp,LOCATION_HAND+LOCATION_GRAVE,0,1,c,e,tp,lvt-lv)
end
...
function c249001164.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(c249001164.spfilter1,tp,LOCATION_HAND+LOCATION_GRAVE,0,1,nil,e,tp,e:GetHandler():GetOriginalLevel()) end
...
function c249001164.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g1=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(c249001164.spfilter1),tp,LOCATION_HAND+LOCATION_GRAVE,0,1,1,nil,e,tp,c:GetOriginalLevel())
	local tc1=g1:GetFirst()
	if not tc1 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g2=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(c249001164.spfilter2),tp,LOCATION_HAND+LOCATION_GRAVE,0,1,1,tc1,e,tp,c:GetOriginalLevel()-tc1:GetLevel())
	g1:Merge(g2)
