...
function cid.initial_effect(c)
	...
	e4:SetCondition(function(e,tp) return not Duel.IsPlayerAffectedByEffect(tp,911630825) end)
	...
	qe:SetCondition(function(e,tp) return Duel.IsPlayerAffectedByEffect(tp,911630825) end)
	c:RegisterEffect(qe)
	--change effect type
...
function cid.mfilter(c,fc,sub,mg,sg)
	return c:IsRace(RACE_ZOMBIE) and (not sg sg:IsExists(Card.IsFusionSetCard,1,nil,0x2e7))
end
