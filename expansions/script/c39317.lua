--Starforce Dimensional Direction
...
function s.filter0(c,e)
	return c:IsType(TYPE_MONSTER) and c:IsCanBeFusionMaterial() and c:IsAbleToGrave() and (not e or not c:IsImmuneToEffect(e))
end
function s.filter2(c,e,tp,m,f,chkf)
	return c:IsType(TYPE_FUSION) and c:IsSetCard(0x24f) and (not f or f(c))
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local chkf=tp
	local mg=Duel.GetMatchingGroup(s.filter0,tp,LOCATION_DECK,0,nil,e)
