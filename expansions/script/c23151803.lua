...
function s.thtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,Duel.SelectTarget(tp,aux.NecroValleyFilter(aux.AND(Card.IsSetCard,Card.IsAbleToHand)),tp,LOCATION_GRAVE,0,1,1,nil,0x9fa),1,0,0)
end
...
function s.tdtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsExistingTarget(s.tdfilter,tp,LOCATION_GRAVE,0,2,nil) and Duel.IsPlayerCanDraw(tp,1) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TODECK)
	local g=Duel.SelectTarget(tp,aux.NecroValleyFilter(s.tdfilter),tp,LOCATION_GRAVE,0,2,2,nil)
	Duel.SetOperationInfo(0,CATEGORY_TODECK,g,#g,0,0)
	Duel.SetOperationInfo(0,CATEGORY_DRAW,nil,0,tp,1)
end
function s.tdop(e,tp,eg,ep,ev,re,r,rp)
	local tg=Duel.GetChainInfo(0,CHAININFO_TARGET_CARDS):Filter(Card.IsRelateToEffect,nil,e)
	if not tg or #tg==0 or Duel.SendtoDeck(tg,nil,0,REASON_EFFECT)==0 then return end
	local g=Duel.GetOperatedGroup()
	for p=tp,1-tp do
		if g:IsExists(aux.AND(Card.IsControler,Card.IsLocation),1,nil,p,LOCATION_DECK) then
			Duel.ShuffleDeck(p)
		end
	end
