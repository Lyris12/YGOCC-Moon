...
Auxiliary.PendCondition = function()
	return	function(e,c,og)
				...
				if c:IsHasEffect(EFFECT_PENDULUM_SUMMON_WITH_ONE_SCALE) then
				...
				if ct==2 then
					...
					if scales:IsExists(aux.NOT(Card.IsType),1,c,TYPE_PENDULUM) then
...
function Auxiliary.PConditionFilter(c,e,tp,lscale,rscale,eset,tg)
	local lv=0
	...
	return	function(e,tp,eg,ep,ev,re,r,rp,c,sg,og)
				...
				if ct==1 or pande then
					...
					for _,ce in ipairs(eset2) do
						...
						if tg:IsExists(Auxiliary.PConditionFilter,1,nil,e,tp,lscale,rscale,eset,targ) then
					...
					if #options>0 then
						...
						if opt>1 or not pandepend then
							...
							if lscale>rscale then lscale,rscale=rscale,lscale end
							tg=tg:Filter(Auxiliary.PConditionFilter,nil,e,tp,lscale,rscale,eset,targ)
