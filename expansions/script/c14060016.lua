...
function cm.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) then
		...
		if Duel.SendtoDeck(c,nil,2,REASON_EFFECT) then
			local gc=Duel.GetFieldGroup(tp,LOCATION_HAND,0):FilterCount(Card.IsAbleToGrave,nil)
			if gc>0 and Duel.SelectYesNo(tp,aux.Stringid(m,2)) then
				Duel.BreakEffect()
				local g=Duel.SelectMatchingCard(tp,Card.IsAbleToGrave,tp,LOCATION_HAND,0,1,gc,nil)
				if #g>0 then
...
function cm.tdfilter(c)
	return (c:IsLocation(LOCATION_GRAVE+LOCATION_REMOVED) or c:IsFaceup()) and c:IsAbleToDeck()
end
function cm.tdtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(cm.tdfilter,tp,LOCATION_GRAVE+LOCATION_REMOVED+LOCATION_EXTRA,0,1,nil) end
...
function cm.tdop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(cm.tdfilter),tp,LOCATION_GRAVE+LOCATION_REMOVED+LOCATION_EXTRA,0,1,3,nil)
