...
function cid.attg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsExistingMatchingCard(cid.cfilter,tp,LOCATION_DECK,0,1,nil,tp)
		and Duel.IsExistingTarget(cid.xfilter,tp,LOCATION_MZONE,0,1,nil) end
...
function cid.atop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) then
		...
		Duel.Overlay(tc,Duel.SelectMatchingCard(tp,cid.cfilter,tp,LOCATION_DECK,0,1,1,nil,tp))
	end
end
