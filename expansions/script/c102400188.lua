...
function s.filter(c)
	return c:IsFaceup() and c:IsSetCard(0xa5f) and c:IsAbleToHand() and not c:IsCode(id)
end
...
function s.sfilter(c,e,tp)
	return c:IsFaceup() and c:IsSetCard(0xa5f) and c:IsCanBeSpecialSummoned(e,0,tp,false,false) and not c:IsCode(id)
end
...
function s.op(e,tp,eg,ep,ev,re,r,rp)
	...
	Duel.SpecialSummon(Duel.SelectMatchingCard(tp,s.sfilter,tp,LOCATION_REMOVED,0,1,1,nil,e,tp),0,tp,tp,false,false,POS_FACEUP)
end
