...
function cid.tg1(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return mc>0 and Duel.GetLocationCount(tp,LOCATION_SZONE)>0 and Duel.IsExistingTarget(cid.filter,tp,LOCATION_GRAVE,0,1,nil,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_EQUIP)
	local g=Duel.SelectTarget(tp,aux.NecroValleyFilter(cid.filter),tp,LOCATION_GRAVE,0,1,mc,nil,tp)
...
function cid.tg2(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and cid.eqfilter2(chkc,tp) end
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_SZONE)>0
		and Duel.IsExistingTarget(cid.eqfilter2,tp,LOCATION_GRAVE,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_EQUIP)
	local g=Duel.SelectTarget(tp,cid.aux.NecroValleyFilter(eqfilter2),tp,LOCATION_GRAVE,0,1,1,nil)
	Duel.SetOperationInfo(0,CATEGORY_EQUIP,g,#g,0,0)
end
...
function cid.discon(e,tp,eg,ep,ev,re,r,rp)
	...
	return (ex1 and tg1~=nil and tc1+tg1:FilterCount(cid.cfilter,nil,tp)-#tg1>0)
		or (ex2 and tg2~=nil and tc2+tg2:FilterCount(cid.cfilter,nil,tp)-#tg2>0)
		or (ex3 and tg3~=nil and tc3+tg3:FilterCount(cid.cfilter,nil,tp)-#tg3>0)
		or (ex4 and tg4~=nil and tc4+tg4:FilterCount(cid.cfilter,nil,tp)-#tg4>0)
		or (ex5 and tg5~=nil and tc5+tg5:FilterCount(cid.cfilter,nil,tp)-#tg5>0)
		or (ex6 and tg6~=nil and tc6+tg6:FilterCount(cid.cfilter,nil,tp)-#tg6>0)
		or (ex7 and tg7~=nil and tc7+tg7:FilterCount(cid.cfilter,nil,tp)-#tg7>0)
end
...
function cid.reptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return c:IsOnField() and c:IsFaceup()
		and eg:IsExists(cid.repfilter,1,nil,tp) and e:GetHandler():GetEquipGroup():IsExists(cid.drepfilter,1,nil,e) end
	if Duel.SelectYesNo(tp,aux.Stringid(id,3)) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESREPLACE)
		local g=e:GetHandler():GetEquipGroup():FilterSelect(tp,cid.drepfilter,1,1,nil)
