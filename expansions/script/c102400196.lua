...
function cid.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		local chkf=tp
		local mg1=Duel.GetFusionMaterial(tp)+Duel.GetMatchingGroup(cid.filter1,tp,LOCATION_DECK,0,nil)
		aux.FCheckAdditional=cid.fcheck
		aux.GCheckAdditional=cid.gcheck
...
function cid.activate(e,tp,eg,ep,ev,re,r,rp)
	local chkf=tp
	local mg1=Duel.GetFusionMaterial(tp):Filter(aux.NOT(Card.IsImmuneToEffect),nil,e)+Duel.GetMatchingGroup(cid.filter1,tp,LOCATION_DECK,0,nil)
	aux.FCheckAdditional=cid.fcheck
	aux.GCheckAdditional=cid.gcheck
