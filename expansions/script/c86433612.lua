...
function c86433612.effectop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) and c:IsFaceup() and tc:IsRelateToEffect(e) and tc:IsFaceup() then
		...
		if typ~=0 then
			...
			e0:SetValue(typ)
			e0:SetReset(RESET_EVENT+RESETS_STANDARD-RESET_TOFIELD-RESET_TURN_SET+RESET_PHASE+PHASE_END)
			tc:RegisterEffect(e0)
			c86433612.remembertype=0
		end
		local cid=tc:CopyEffect(val,RESET_EVENT+RESETS_STANDARD-RESET_TOFIELD-RESET_TURN_SET+RESET_PHASE+PHASE_END,1)
		local e1=Effect.CreateEffect(c)
