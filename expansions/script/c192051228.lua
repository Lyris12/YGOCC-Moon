...
function s.valcheck(e,c)
	if c:GetMaterial():IsExists(s.mtfilter,1,nil,e:GetHandler():GetFirstCardTarget()) then
...
function s.desop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and tc:IsLocation(LOCATION_MZONE) then
		if Duel.Destroy(tc,REASON_EFFECT)>0 and Duel.IsExistingMatchingCard(s.thfilter,tp,LOCATION_DECK,0,1,nil) then
