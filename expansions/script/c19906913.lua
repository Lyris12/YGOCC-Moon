...
function s.destg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsExistingTarget(s.xfilter1,tp,LOCATION_PZONE,0,1,c) and Duel.IsExistingMatchingCard(s.filter,tp,0,LOCATION_MZONE,1,nil)end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g=Duel.SelectTarget(tp,s.xfilter1,tp,LOCATION_PZONE,0,1,1,c)
...
local exc=nil
	...
	local g=Duel.SelectMatchingCard(tp,s.filter,tp,0,LOCATION_MZONE,1,1,exc)
	if #g>0 then
		local tc=g:GetFirst()
...
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_REMOVED+LOCATION_EXTRA,0,1,nil,e,tp) end
