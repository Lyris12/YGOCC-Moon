...
function c249000687.initial_effect(c)
	...
	e3:SetCost(c249000687.thcost)
	e3:SetTarget(c249000687.thtg)
	e3:SetOperation(c249000687.thop)
	c:RegisterEffect(e3)
end
...
function c249000687.thcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():IsReleasable() end
	Duel.Release(e:GetHandler(),REASON_COST)
end
function c249000687.thfilter(c)
	return c:IsSetCard(0x1E4) and c:IsType(TYPE_SPELL) and c:IsAbleToHand()
end
function c249000687.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c249000687.thfilter,tp,LOCATION_DECK,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_DECK)
end
function c249000687.thop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectMatchingCard(tp,c249000687.thfilter,tp,LOCATION_DECK,0,1,1,nil)
